...
use anyhow::Result;
use rdx_stderr::{init_from_env, Level as LogLevel, set_level};
use crate::{log_info};
    ...
    match command {
        ...
        _ => {
            ...
            match command {
                ...
                cli::Command::Import { file_path, mode, map_base, map_proj, map_ds, format } => commands::import::execute(&file_path, &mode, &map_base, &map_proj, &map_ds, &format, &database, resolved_ids)?,
                cli::Command::Export { file_path, format, proj, ds, vs, doc, key, seg } => commands::export::execute(&file_path, format, (proj,ds,vs,doc,key,seg), &database, resolved_ids)?,
                cli::Command::Migrate { dry_run, backup } => commands::migrate::execute(dry_run, backup, &database, resolved_ids)?,
                _ => unreachable!(),
            }
        }
    }
