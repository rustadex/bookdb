...
pub fn execute(dry_run: bool, db: &Database, ids: ResolvedContextIds) -> Result<()> {
    ...
    if chunks.is_empty() { log_info!("no legacy doc_chunks found; nothing to migrate"); return Ok(()); }
    for (doc_key, value) in chunks {
        log_debug!("migrate {} -> docs/_root", doc_key);
        if !dry_run { db.set_doc_segment(&doc_key, "_root", "text/plain", value.as_bytes(), ds_id)?; }
    }
